...
pub struct ProcessesTab {
	...
	pub cum_cache: Rc<RefCell<Option<std::collections::HashMap<i32, state::CumulativeResources>>>>,
	...
	pub fn new(
		...
		Self {
			...
			view_state: Rc::new(RefCell::new(state::ViewState {
				...
				pid_filter_mode: config.processes.pid_filter_mode,
				search: String::new(),
				sort_col: 5,
				sort_dir: gpui_component::table::ColumnSort::Descending,
				resource_view_mode: config.processes.resource_view_mode,
				cached: None,
			})),
			table_state: None,
			input_state: None,
			clear_search_on_pin: config.processes.clear_search_on_pin,
