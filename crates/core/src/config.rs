...
impl Config {
	pub fn apply_override(&mut self, ron_str: &str) -> Result<()> {
		...
		crate::merge::deep_merge(&mut base_value, &overlay_value);
...
mod tests {
	...
	fn test_apply_override_refresh_ms() {
		...
		assert_eq!(cfg.general.interface.theme, "Default Dark".to_string());
	}
	...
	fn test_apply_override_partial_enum_field() {
		let mut cfg = Config::default();
		cfg.general.interface.theme = "Catppuccin Latte".to_string();
