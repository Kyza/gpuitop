use crate::processes::delegate::ProcessTableDelegate;
use gpui::*;
...
impl ProcessTableDelegate {
	...
	pub fn build_tree(&self) -> TreeData {
		...
		let mut proc_matches = |proc: &ProcessSnapshot| -> bool {
			...
			if has_search {
				if !gpuitop_core::fuzzy::fuzzy_match(
					&search_term,
					&proc.name.to_lowercase(),
					&mut matcher,
				) && !proc.pid.to_string().contains(&search_term)
					&& !gpuitop_core::fuzzy::fuzzy_match(
						&search_term,
		...
		let display_pids = gpuitop_core::processes::tree::with_ancestors(
			&matched_pids,
			&pid_to_ppid,
		);
		...
		let expand_pids = gpuitop_core::processes::tree::ancestors_to_expand(
			&matched_pids,
			&pid_lookup,
		);
