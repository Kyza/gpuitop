...
use crate::processes::ProcessesTab;
use gpui::prelude::*;
use gpui::*;
use gpui_component::{
					...
					.child({
						...
						.suffix(
							div()
								.flex()
								.flex_row()
								.when(gpuitop_window_picker::is_window_picker_available(), |el| {
									el.child({
									...
									.on_click(cx.listener(
										move |_, _, _, cx| {
											...
											std::thread::spawn(move || {
												*r2.lock().unwrap() =
													gpuitop_window_picker::pick_window();
												p2.store(
													false,
													std::sync::atomic::Ordering::Relaxed,
												);
					...
					div()
						...
						.child({
							...
							for (i, f) in type_filters.iter().enumerate() {
								let icon_name = crate::theme::filter_icon(f);
								let tooltip = match f {
									Filter::Gui => {
								...
								let color = crate::theme::filter_color(f, cx);
								let mut t =
									...
									.dropdown_menu(
										move |mut menu, _window, _cx| {
											...
											for c in all_states.iter().filter(
												...
												menu = menu.item(crate::processes::chips::get_state_item(
													*c,
													&active,
													vs.clone(),
												));
