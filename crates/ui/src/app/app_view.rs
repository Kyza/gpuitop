	...
	pub fn new(
		...
		let theme_observer = cx
			.observe_global::<gpui_component::theme::ThemeRegistry>(
				|this, cx| {
					crate::themes::unpack_builtins_to_disk();
					...
					crate::themes::load_builtins_into_registry(cx);
					...
					let still_missing =
						!gpui_component::theme::ThemeRegistry::global(cx)
							...
							.contains_key(&active_name);
					if still_missing {
						crate::themes::apply_theme_by_name(
	...
	.on_click(
		move |_: &ClickEvent, window: &mut Window, cx: &mut GpuiApp| {
			*click_preview.borrow_mut() = None;
			...
			crate::themes::apply_theme_by_name(&item_name, Some(window), cx);
		},
	);
	menu.item(item)
}
...
pub(crate) fn build_theme_menu(
	...
	let families = crate::themes::list_theme_families(cx);
